#!/usr/bin/env python3
"""
YOLOv8/v11/v12 GPU (Metal/MPS) FP16 Benchmark Script

Benchmarks FPS using Metal Performance Shaders (GPU) with FP16 precision.
"""
...
def benchmark_model_on_video(model_path: Path, video_path: Path, warmup: int = 10, max_frames: int = None) -> dict:
    """Benchmark a model on video frames using GPU (MPS) with FP16."""
    results = {
    ...
    try:
        # Load model - Ultralytics handles FP16 via the half parameter
        ...
        while frame_count < frames_to_process:
            ...
            # Run inference on MPS (GPU) with FP16
            start = time.perf_counter()
            _ = model(frame, imgsz=INPUT_SIZE, device='mps', half=True, verbose=False)
            # Synchronize GPU
...
def run_benchmarks():
    ...
    all_results = {
        ...
        'device': 'mps',
        'precision': 'fp16',
        'device_name': 'Apple GPU (Metal) FP16',
        'models': {},
    }
    ...
    print("=" * 60)
    print("YOLO GPU (Metal/MPS) FP16 Benchmark")
    print("=" * 60)
    ...
    for family in MODEL_FAMILIES.keys():
        ...
        for variant in MODEL_VARIANTS:
            ...
            # Benchmark on GPU with FP16
            print(f"  [BENCHMARK] {model_name} on GPU (FP16)...")
    ...
    # Save results
    results_file = RESULTS_DIR / f"benchmark_gpu_fp16_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def print_summary(results: dict):
    ...
    print("\n" + "=" * 80)
    print("GPU FP16 BENCHMARK RESULTS SUMMARY")
    print("=" * 80)
