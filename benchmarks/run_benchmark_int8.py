#!/usr/bin/env python3
"""
YOLOv8/v11 INT8 CoreML Benchmark Script

Benchmarks INT8 quantized models on Apple Neural Engine.
"""
...
def benchmark_model_on_video(model_path: Path, video_path: Path, warmup: int = 10, max_frames: int = None) -> dict:
    """Benchmark an INT8 model on video frames."""
    results = {
...
def run_benchmarks():
    ...
    for family in MODEL_FAMILIES.keys():
        ...
        for variant in MODEL_VARIANTS:
            ...
            int8_path = MODELS_DIR / family / f"{model_name}_int8.mlpackage"

            if not int8_path.exists():
                print(f"\n  {model_name}_int8: Model not found, skipping")
                continue

            print(f"\n  Benchmarking {model_name}_int8...")

            # Benchmark
            print(f"  [BENCHMARK] {model_name}_int8...")
    ...
    # Save results
    results_file = RESULTS_DIR / f"benchmark_int8_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def print_summary(results: dict):
    ...
    print("\n" + "=" * 80)
    print("INT8 ANE BENCHMARK RESULTS SUMMARY")
    print("=" * 80)
    ...
    for family in ['yolov8', 'yolov11']:
        ...
        for variant in MODEL_VARIANTS:
            ...
            model_name = f"{MODEL_FAMILIES[family]}{variant}_int8"
